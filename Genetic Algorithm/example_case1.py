...
def Function(X):       # to find the maximum of this function
...
class MGA(object):
    ...
    def evolve(self, n):    # nature selection wrt pop's fitness
        for _ in range(n):  # random pick and compare n times
            ...
            product = Function(self.translateDNA(sub_pop))
            fitness = get_fitness(product)
        ...
        DNA_pro = self.translateDNA(self.pop)
        pre = Function(DNA_pro)
        return DNA_pro, pre
...
for _ in range(N_GENERATIONS):                    # 100 generations
    DNA_prod, predict = ga.evolve(10)          # natural selection, crossover and mutation

    # something about plotting
    if 'sca' in globals():
        sca.remove()
    sca = plt.scatter(DNA_prod, predict, s=100, lw=0, c='red', alpha=0.5)
    plt.pause(0.1)
