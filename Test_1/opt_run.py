...
import math
from Test_1 import testCase
from Test_1 import opt_utils

plt.rcParams['figure.figsize'] = (7.0, 4.0)
plt.rcParams['image.interpolation'] = 'nearest'
plt.rcParams['image.cmap'] = 'gray'

def update_parameters_with_gd(parameter, grad, learning_rated):
    """
    使用梯度下降更新参数
    参数：
    ...
    """
    L = len(parameter) // 2 #神经网络的层数
    #更新每个参数
    for l in range(L):
        parameter["W" + str(l + 1)] = parameter["W" + str(l + 1)] - learning_rated * grad["dW" + str(l + 1)]
        parameter["b" + str(l + 1)] = parameter["b" + str(l + 1)] - learning_rated * grad["db" + str(l + 1)]
...
def random_mini_batches(X, Y, mini_batch_sizes=64, seed=0):
    """
    ...
    #第二步，分割
    num_complete_miniBatch = math.floor(m / mini_batch_sizes) #把你的训练集分割成多少份,请注意，如果值是99.99，那么返回值是99，剩下的0.99会被舍弃
    for k in range(0,num_complete_miniBatch):
        ...
        mini_batched.append(mini_batch)

    #如果训练集的大小刚好是mini_batch_size的整数倍，那么这里已经处理完了
    #如果训练集的大小不是mini_batch_size的整数倍，那么最后肯定会剩下一些，我们要把它处理了
    if m % mini_batch_sizes != 0:
        #获取最后剩余的部分
        mini_batch_X = shuffled_X[:, mini_batch_sizes * num_complete_miniBatch:]
        mini_batch_Y = shuffled_Y[:, mini_batch_sizes * num_complete_miniBatch:]
...
def initialize_velocity(parameter):
    ...
    for l in range(L):
        v["dW" + str(l + 1)] = np.zeros_like(parameter["W" + str(l + 1)])
        v["db" + str(l + 1)] = np.zeros_like(parameter["b" + str(l + 1)])
...
def update_parameters_with_momentun(parameter, grad, v, beta, learning_rated):
    """
    使用动量更新参数
    参数：
    ...
    for l in range(L):
        #计算速度
        v["dW" + str(l + 1)] = beta * v["dW" + str(l + 1)] + (1 - beta) * grad["dW" + str(l + 1)]
        v["db" + str(l + 1)] = beta * v["db" + str(l + 1)] + (1 - beta) * grad["db" + str(l + 1)]

        #更新参数
        parameter["W" + str(l + 1)] = parameter["W" + str(l + 1)] - learning_rate * v["dW" + str(l + 1)]
        parameter["b" + str(l + 1)] = parameter["b" + str(l + 1)] - learning_rate * v["db" + str(l + 1)]
...
def initialize_adam(parameter):
    ...
    for l in range(L):
        ...
        s["dW" + str(l + 1)] = np.zeros_like(parameter["W" + str(l + 1)])
        s["db" + str(l + 1)] = np.zeros_like(parameter["b" + str(l + 1)])

    return v,s

def update_parameters_with_adam(parameter, grad, v, s, t, learning_rated=0.01, beta1=0.9, beta2=0.999, epsilon=1e-8):
    """
    使用Adam更新参数
    ...
    for l in range(L):
        ...
        s["dW" + str(l + 1)] = beta2 * s["dW" + str(l + 1)] + (1 - beta2) * np.square(grad["dW" + str(l + 1)])
        s["db" + str(l + 1)] = beta2 * s["db" + str(l + 1)] + (1 - beta2) * np.square(grad["db" + str(l + 1)])
        ...
        parameter["W" + str(l + 1)] = parameter["W" + str(l + 1)] - learning_rated * (v_corrected["dW" + str(l + 1)] / np.sqrt(s_corrected["dW" + str(l + 1)] + epsilon))
        parameter["b" + str(l + 1)] = parameter["b" + str(l + 1)] - learning_rated * (v_corrected["db" + str(l + 1)] / np.sqrt(s_corrected["db" + str(l + 1)] + epsilon))
...
def model(X, Y, layers_dim, optimizer, learning_rated=0.0007,
          mini_batch_sized=64, beta=0.9, beta1=0.9, beta2=0.999,
    ...
    #初始化参数
    parameter = opt_utils.initialize_parameters(layers_dim)

    #选择优化器
    if optimizer == "gd":
        pass #不使用任何优化器，直接使用梯度下降法
    elif optimizer == "momentum":
        v = initialize_velocity(parameter) #使用动量
    elif optimizer == "adam":
        v, s = initialize_adam(parameter)#使用Adam优化
    else:
    ...
    for i in range(num_epochs):
        ...
        seed = seed + 1
        minibatches = random_mini_batches(X, Y, mini_batch_sized, seed)
        ...
        for minibatch in minibatches:
            ...
            #前向传播
            A3 , cache = opt_utils.forward_propagation(minibatch_X, parameter)
            ...
            #更新参数
            if optimizer == "gd":
                parameter = update_parameters_with_gd(parameter, grads, learning_rated)
            elif optimizer == "momentum":
                parameter, v = update_parameters_with_momentun(parameter, grads, v, beta, learning_rated)
            elif optimizer == "adam":
                t = t + 1
                parameter , v , s = update_parameters_with_adam(parameter, grads, v, s, t, learning_rated, beta1, beta2, epsilon)
        #记录误差值
        if i % 100 == 0:
            costs.append(cost)
            #是否打印误差值
    ...
    if is_plot:
        ...
        plt.title("Learning rate = " + str(learning_rated))
        plt.show()
...
#绘制分类图
plt.title("Model with Momentum Gradient Descent and Adam optimization")
axes = plt.gca()
